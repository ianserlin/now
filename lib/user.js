...
exports.initialize = function (nowjs) {
  var fn = require('./function').init(nowjs);
	var middleware = nowjs.middleware;
	var beforeware = nowjs.beforeware;
  ...
  var User = function (socket) {
    ...
    this.now = Proxy.wrap(this);
		...
		var index = 0;
		var stack = beforeware;
		function nextBefore(){
			var layer = stack[index++];
			if(!layer){return}
			layer.handle(self,nextBefore);
		}
		nextBefore();
	      ...
	      for (var i = 0, ll = args.length; i < ll; i++) {
	        if (nowUtil.hasProperty(args[i], 'fqn')) {
			  ...
			  function next(err) {
			    ...
			    // all done
			    if (!layer) {
						console.log( 'calling original now function after passing through ' + (index-1) + ' middlewares' );
			      theFunction.apply(client, args);
						return;
			    }
			    ...
			    try {
			      ...
			      if (err) {
			        if (arity === 4) {
			          layer.handle(err, self, args, next);
			        } else {
			          next(err);
			        }
			      } else if (arity < 4) {
			        layer.handle(self, args, next);
			      } else {
			        next();
			      }
			    } catch (e) {
